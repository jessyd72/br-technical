...
def logPrint(text, text_type='', err_info = ''):
    ...
    if text_type == 'info':
        ...
        logging.info(text)
        logging.error('text_type parameter is not recognized! Check the script!!')
...
def getFiresByCountry(in_csv, x, y, conf_lvl, in_bnd, bnd_id, out_path, findAvgDist = True, findNearBorder = True):
    ...
    logPrint('Converting rows to points...')
    ...
    if check_cnt > 0:
        logPrint(f'{str(check_cnt)} point(s) fall outside of boundaries!', 'warn')
        logPrint('These data will not be included when summing by intersection!', 'warn')

    logPrint('Finding boundary shapes containing points...')
    bnd_w_fire = arcpy.SelectLayerByLocation_management(in_bnd, 'CONTAINS', fires_pnt, selection_type='NEW_SELECTION')

    logPrint('Finding point count and maximum confidence level by boundary...')
    ...
    logPrint(f'Summarizing by {bnd_id}')
    ...
    if findNearBorder:
        ...
        logPrint('Finding fires within a distance to a border...')
    
        def getFiresWithinDistToBorder(pnts, bnds, dist=5, unit='KILOMETERS'):

            logPrint('Converting boundaries to lines...')
            bnd_lines = arcpy.PolygonToLine_management(bnds, r'in_memory\bnd_lines')

            logPrint(f'Finding points within {str(dist)} {unit.lower()} to a border...')
...
if __name__ == '__main__':
    ...
    try:
        ...
        logPrint('Ensure all previously created CSVs are closed!')
        logPrint('Check that supporting data is not open in another program (ArcGIS Pro/Map/Catalog)!')

    except Exception as e:
        logPrint("EXCEPTION OCCURRED", 'err', e)

    logPrint('Finished run')
    logPrint('---------------------------------------------------------\n')
